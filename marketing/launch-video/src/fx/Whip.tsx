...
export const Whip: React.FC<{ children: React.ReactNode; dur: number; inFrames?: number; outFrames?: number; id: string; dir?: 1 | -1 }> = ({
  ...
  const xin = (1 - ease.outExpo(pin)) * 1500;
  const xout = -ease.inExpo(pout) * 1700;
  const vin = frame < inFrames ? Math.abs(1500 * (ease.outExpo(clamp((frame + 1) / inFrames)) - ease.outExpo(pin))) : 0;
  const vout = pout > 0 ? Math.abs(1700 * (ease.inExpo(clamp((frame + 1 - (dur - outFrames)) / outFrames)) - ease.inExpo(pout))) : 0;
