...
enum FactExtractionPrompt {
    ...
    - WHO ACTS decides the direction — never who benefits. A message where the SENDER commits to do something ("Will check with Deeksha", "creating this", "I'll send it tomorrow") is THEIR commitment → owes_me (subject = sender), NEVER i_owe. i_owe requires [ME] to be the actor: either the message asks [ME] to do it, or [ME] committed in [ME]'s own message ("I'll…", "lemme see", "will do"). A joint "we should / we will have to…" with no explicit owner is NOT [ME]'s task — skip it unless [ME] explicitly takes it (and if the OTHER person takes it, it's owes_me).
      ...
      * [ME]: "Are the Armoriq graphics ready?" / THEM: "No, asking Piyush" → owes_me, subject = THEM, object = "Armoriq graphics readiness", action = "Wait for them to confirm whether the Armoriq graphics are ready". Their terse reply means THEY undertook the investigation and still owe the answer; a later [ME] "ok thanks" only acknowledges that undertaking and does not deliver the answer.
    ...
    - DM OR PERSON-AUTHORED EMAIL ONLY — A DROPPED CONVERSATION IS A REPLY [ME] OWES. GATE FIRST: this rule applies ONLY when the VERY LAST message in the transcript is from the other person. For email, it applies only to a human conversation—not newsletters, receipts, OTPs, security alerts, marketing, automated notifications, or no-reply senders. If the last message is from [ME], STOP — this rule is off, and the normal direction rules alone decide. ([ME] having answered is exactly what "not dropped" means; a [ME] reply that also asks them for something is an ordinary ask, not a dropped thread.)
      ...
      Judge only [ME]'s SILENCE, never the age of the message. A DM from an hour ago with no reply is as much [ME]'s loop as one from last month; the queue decides what is urgent, you decide what is open.
    ...
    - CLOSING LOOPS — match the closer to the loop's DIRECTION. An i_owe loop closes only when a NEW message from [ME] actually ADDRESSES it — answers that exact question, sends that exact thing, gives that exact update. An owes_me loop closes only when a NEW message from THE OTHER PERSON actually DELIVERS what [ME] was waiting on — they share the thing, answer the question, give the update. Put the closed loop's number in resolvedLoops and do NOT re-emit it. A message about something else closes NOTHING — match the reply to the loop; never close on unrelated chatter.
    - THE BALL FLIPS: when their delivery closes an owes_me loop and [ME] has not replied to it by the end of the transcript, the conversation now sits with [ME] — also emit a NEW i_owe loop with kind "reply" for responding to that delivery (subject = that person, sourceMsg = their delivery message). [ME] asked, they answered, now it is [ME]'s turn. Skip the flip only when their delivery clearly ended the exchange (a bare "done"/"sent"/link drop needing no response) or [ME] already replied to it.
    ...
    BEFORE YOU OUTPUT — run this checklist on EVERY open loop you are about to emit. These four are where mistakes actually happen, so they are repeated here deliberately; a loop that fails any of them must be corrected or dropped.
      1. ACTOR: who does the work? [ME] → "i_owe". The other person → "owes_me". A name or @handle at the start of a message is only who is being ADDRESSED — never assume it is the actor. ACCEPTING A TASK KEEPS IT: when [ME] agrees to do something and merely asks for its input ("sure — send it over", "ok, share the file and I'll review"), the actor is STILL [ME] → i_owe. Waiting on materials does not hand the task to the other person; asking for the input is part of doing the work.
