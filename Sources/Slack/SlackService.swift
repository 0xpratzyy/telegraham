...
final class SlackService: ObservableObject, MessageSource {
    ...
    func chatHistory(chatId: Int64, fromMessageId: Int64, limit: Int) async throws -> [TGMessage] {
        // UI/search callers read the local corpus. Slack's non-Marketplace
        // history endpoint allows roughly one request/minute; doing network IO
        // here made two generic index workers reserve minutes of API slots and
        // caused every Slack screen to appear frozen. The dedicated refresh
        // loop below is now the only owner of remote history pagination.
        let records = await db.loadMessageHistoryPage(
            chatId: chatId,
            beforeMessageId: fromMessageId,
            limit: limit
        )
        return records.map { MessageCacheService.CachedMessage.from($0).toTGMessage() }
    }
    ...
    private func backfillOlder(_ chat: TGChat) async {
        ...
        guard let native = try? await db.nativeId(source: sourceID, intId: oldest.id),
              let latest = Self.ts(fromMessageNative: native) else {
            fullyDeepened.insert(chat.id)
            return
        }
        let older = (try? await fetchHistory(chat: chat, oldest: nil, latest: latest)) ?? []
    ...
    private func fetchHistory(chat: TGChat, oldest: String?, latest: String? = nil) async throws -> [TGMessage] {
        await ensureValidToken()
