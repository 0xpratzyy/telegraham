...
- `done` validated that the script harness reads the same `com.pidgy.aiApiKey` credential file as the app and can complete live `gpt-5.4-mini` runs from it
...
- `in_progress` validate whether the current offline winner `field_aware_groups_v3_private_recall_v2_digest_v5_4x12` should replace the live product variant after more real usage
...
3. `next` build the same final-answer oracle harness for topic-search quality before promoting topic-search changes into product
...
- `later` replace file-based AI credential persistence with real macOS Keychain storage, provider-scoped keys, and session-only-by-default behavior
